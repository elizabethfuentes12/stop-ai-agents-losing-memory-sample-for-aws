...
**Problem:** Semantic memory retrieves by *similarity*, so it finds related pieces but can't connect them. A multi-hop question — "Who did I meet that's connected to boutique hotels in Japan?" — needs the *relationships* between memories, not just their vectors.
...
> **Assumed familiarity:** This is the most advanced demo in the series. It assumes you're comfortable with the earlier demos (agent state, semantic retrieval) and have a Neo4j instance you can connect to. If you're new here, start with [Demo 01](../01-memory-decay-demo/).
...
**The question:** *"Who did I meet that's connected to boutique hotels in Japan?"*

The answer — **Sarah Chen** — is never stated directly. You can only reach it by following the relationships. That's a multi-hop question, and it's exactly where similarity-only memory falls short.
...
|-----------|----------|----------------------------------|
| **Before** — `VectorRetriever` | Pure vector similarity | Surfaces `boutique`, `Kyoto`, `Japan` as separate pieces. **Never connects them to Sarah.** |
| **After** — `VectorCypherRetriever` | Similarity → graph traversal | Finds an entry node, walks the edges back: **Sarah Chen → Vista Hotels → boutique → Kyoto → Japan.** |
...
**AI Model Provider** — This demo uses OpenAI by default. You can also use [Amazon Bedrock](https://aws.amazon.com/bedrock/), Anthropic, or Ollama. See [supported model providers](https://strandsagents.com/docs/user-guide/concepts/model-providers/). Change the model in `test_graph_memory.py`.
...
| Semantic (Demo 03) | Blobs + vectors | Yes (top-k) | No — no notion of relationships |
...
| Embeddings | OpenAI `text-embedding-3-small` (real) | Same, or Amazon Titan via Bedrock (Demo 07) |
...
2. [Demo 02: Core Memory](../02-core-memory-demo/) — Agent-managed structured memory
3. [Demo 03: Memory Retrieval](../03-memory-retrieval-demo/) — Semantic search over memory
