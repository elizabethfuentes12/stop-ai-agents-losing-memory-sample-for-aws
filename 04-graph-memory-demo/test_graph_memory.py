...
def run_test_1_semantic(driver, db, embedder):
    ...
    recovered = any("Sarah Chen" in item.content for item in result.items)
    print(f"\n  Recovers the person (Sarah Chen)? {recovered}")
    print("  It surfaces boutique / Kyoto / Japan as separate pieces but cannot connect")
...
def run_test_2_graph(driver, db, embedder):
    ...
    print("  person: Sarah Chen -> Vista Hotels -> boutique -> Kyoto -> Japan.")
...
def run_test_3_agent(driver, db, embedder):
    ...
    teach = "By the way, remember that Sarah Chen works at Vista Hotels — she's my contact there."
    print(f"\nTurn 2 (write): {teach}")
    resp = agent(teach)
