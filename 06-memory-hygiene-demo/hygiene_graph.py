...
Neo4j specifics (verified against neo4j-graphrag 1.18.0 + Neo4j 2026.01.3, same as Demo 04):
...
# Legitimate memory graph: a real boutique hotel the traveler knows.
LEGIT_FACTS = [
...
# The poison: a fake hotel wired in as a boutique place in Kyoto/Japan. It arrives inside
# an injection-style message from a low-trust source (see POISON_CONTENT below).
POISON_ENTITY = "Scam Resort"
POISON_FACTS = [
    ("Scam Resort", "HAS_STYLE", "boutique"),
    ("Scam Resort", "LOCATED_IN", "Kyoto"),
]
...
NODE_TEXT = {
    ...
    "Scam Resort": "Scam Resort. A hotel brand.",
}
...
BLAST_RADIUS_QUESTIONS = [
    "What boutique hotels do I know about?",
    "What hotels do I know in Kyoto?",
    "What hotels do I know in Japan?",
    "Which boutique places have I saved in Japan?",
]

ALLOWED_RELATIONS = {"HAS_STYLE", "LOCATED_IN", "IN_COUNTRY"}

# Traversal for the retriever: from the vector-matched entry node, walk to any hotel brand
# (a node that HAS_STYLE) connected to it, and return that hotel's name.
RETRIEVAL_QUERY = """
WITH node AS entry, score
MATCH (hotel:Memory)-[:HAS_STYLE]->(:Memory)
MATCH path = shortestPath((hotel)-[*0..4]-(entry))
RETURN hotel.name AS hotel, max(score) AS score
ORDER BY score DESC
"""
...
def ensure_database(driver) -> str:
    ...
    Same approach as Demo 04: on servers where the retrievers emit the ``SEARCH`` clause
...
def blast_radius(driver, db: str, embedder) -> dict:
    ...
    Deterministic — checked against the retriever's returned hotel names, no LLM judge.
    """
